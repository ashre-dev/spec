...
def create_vendor_app(vendor_id: str) -> FastAPI:
    ...
    def pay(req: PayRequest):
        if not verify_payment(req.payment_id, req.amount, cfg["price_per_query"]):
            raise HTTPException(
                status_code=400,
                detail=(
                    f"Payment rejected. Required {cfg['price_per_query']}, "
                    f"got {req.amount!r} with payment_id {req.payment_id!r}"
                ),
            )
        token, expires_in = issue_token(req.payer_id)
